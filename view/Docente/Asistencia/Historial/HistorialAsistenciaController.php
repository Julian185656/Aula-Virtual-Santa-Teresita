...
class HistorialAsistenciaController
{
    ...
    public function index()
    {
        ...
        if ($cursoId > 0 && $usuarioId > 0) {
            $resultado = $this->model->obtenerHistorialAlumno($cursoId, $usuarioId, $fechaDesde, $fechaHasta, $pagina, 15);

            $historial = $resultado['historial'];
            $resumen   = $resultado['resumen'];

            $totalPaginas = ($resultado['total'] > 0) ? (int)ceil($resultado['total'] / 15) : 1;
