...
if (!empty($_POST['correo']) && !empty($_POST['codigo']) && !empty($_POST['nuevaContrasena'])) {
    ...
    $stmt = $pdo->prepare("SELECT CodigoRecuperacion, ExpiracionRecuperacion 
    ...
    if ($usuario) {
        ...
        if ($usuario['CodigoRecuperacion'] === $codigo) {
            ...
            if (new DateTime() < new DateTime($usuario['ExpiracionRecuperacion'])) {
                ...
                cambiarContrasenna($correo, $passwordHash);
...
<div class="alert alert-<?= $tipoMensaje ?> alert-custom">
...
<a href="Login.php" class="btn btn-primary btn-block mt-3">
VOLVER AL LOGIN
</a>
